...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_r = tf.get_variable(name = 'U_r', shape = [self._state_size, self._state_size], dtype = tf.float32, initializer=initializer)
            W_r = tf.get_variable('W_r', shape=[x_t.get_shape()[1], self._state_size], dtype=tf.float32, initializer=initializer)
            b_r = tf.get_variable('b_r', shape=[self._state_size], dtype=tf.float32)

            U_z = tf.get_variable(name = 'U_z', shape = [self._state_size, self._state_size], dtype = tf.float32, initializer=initializer)
            W_z = tf.get_variable('W_z', shape=[x_t.get_shape()[1], self._state_size], dtype=tf.float32, initializer=initializer)
            b_z = tf.get_variable('b_z', shape=[self._state_size], dtype=tf.float32)

            U_o = tf.get_variable(name = 'U_o', shape = [self._state_size, self._state_size], dtype = tf.float32, initializer=initializer)
            W_o = tf.get_variable('W_o', shape=[x_t.get_shape()[1], self._state_size], dtype=tf.float32, initializer=initializer)
